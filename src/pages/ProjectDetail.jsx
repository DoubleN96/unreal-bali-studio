...
const ProjectDetail = () => {
    ...
    return (
        <div className="bg-[#050505] text-[#f7f8f5] selection:bg-[#80f20d] selection:text-[#050505] min-h-screen font-serif">
            {/* Cinematic Gallery */}
            <section className="relative h-[75vh] w-full overflow-hidden">
                ...
                <div className="absolute bottom-12 left-6 md:left-12 right-6 flex justify-between items-end">
                    <div className="text-left animate-in slide-in-from-left-10 duration-700">
                        <span className="bg-[#80f20d] text-black text-[10px] uppercase font-bold px-3 py-1.5 rounded-full tracking-wide mb-4 inline-block font-sans">{project.property_type}</span>
                        <h1 className="text-4xl md:text-6xl font-bold text-white mb-2 leading-tight">{project.name}</h1>
                        <div className="flex items-center text-gray-400 text-sm font-medium font-sans">
                    ...
                    <div className="flex gap-2">
                        {allImages.map((_, i) => (
                            <button key={i} onClick={() => setActiveImage(i)} className={`w-2.5 h-2.5 rounded-full transition-all ${i === activeImage ? 'bg-[#80f20d] w-8' : 'bg-white/30'}`} />
                        ))}
                    </div>
                </div>
            </section>
            ...
            {/* Metrics Bar */}
            <div className="bg-[#80f20d] text-[#050505] py-8 px-6 md:px-12 shadow-xl relative z-10">
            ...
            <main className="max-w-7xl mx-auto px-6 md:px-12 py-20 grid grid-cols-1 lg:grid-cols-12 gap-16">
                <div className="lg:col-span-8 space-y-20 text-left">
                    <section>
                        ...
                        <div className="prose prose-invert prose-lg text-gray-400 font-light font-sans max-w-none">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-12">
                            {[
                                ...
                                <div key={idx} className="flex items-center gap-4 bg-white/[0.03] p-6 rounded-2xl border border-white/[0.05]">
                                    <div className="bg-[#80f20d]/10 p-3 rounded-xl">
                                        <span className="material-symbols-outlined text-[#80f20d]">{item.icon}</span>
                                    </div>
                                    <div>
                                        <p className="text-[9px] uppercase font-bold text-gray-500 tracking-widest font-sans">{item.label}</p>
                    ...
                    <section className="bg-white/[0.03] p-8 md:p-12 rounded-[2rem] border border-white/[0.05]">
                        <h3 className="text-3xl font-bold text-white mb-8">Investment Structure</h3>
                        <div className="space-y-8">
                            <div>
                                <div className="flex justify-between text-sm mb-3 font-sans">
                                    <span className="text-gray-400">Capital Appreciation</span>
                            ...
                            {project.investor_tiers && project.investor_tiers.map((tier, idx) => (
                                <div key={idx} className="flex justify-between items-center py-4 border-b border-white/5 last:border-0 font-sans">
                    ...
                    {/* Gallery Grid */}
                    <section>
                        <h3 className="text-3xl font-bold text-white mb-12">Architecture & Design</h3>
                        <div className="grid grid-cols-2 gap-4">
                            {allImages.slice(1).map((img, i) => (
                                <div key={i} className={`rounded-2xl overflow-hidden shadow-2xl ${i % 3 === 0 ? 'col-span-2 aspect-video' : 'aspect-square'}`}>
                ...
                <div className="lg:col-span-4 relative">
                    <div className="sticky top-24 space-y-6">
                        <div className="bg-white/[0.03] backdrop-blur-xl p-8 rounded-[2rem] border border-white/[0.08] shadow-2xl text-left">
                            <h3 className="text-2xl font-bold text-white mb-8 pb-4 border-b border-white/5">Reserve Asset</h3>
                            <div className="space-y-6 mb-10">
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400 font-sans">Investment Yield</span>
                                    <span className="text-[#80f20d] font-bold text-xl">{project.roi}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400 font-sans">Leasehold</span>
                                    <span className="text-white font-bold">{project.contract_years} Years</span>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <a href="https://wa.me/6285217790692" target="_blank" rel="noopener noreferrer" className="w-full bg-[#80f20d] text-black py-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 hover:brightness-110 transition-all font-sans">
                                    <span className="material-symbols-outlined">chat</span> WhatsApp Expert
                                </a>
                                <button className="w-full bg-white/5 text-white py-4 rounded-xl font-bold border border-white/10 flex items-center justify-center gap-2 hover:bg-white/10 transition-all font-sans">
                                    Request Brochure
                                </button>
                            </div>
                            <p className="text-[10px] text-gray-500 mt-6 text-center uppercase tracking-widest font-sans">Secured by PMA Notary Laws</p>
                        </div>
                    </div>
                </div>
            </main>
